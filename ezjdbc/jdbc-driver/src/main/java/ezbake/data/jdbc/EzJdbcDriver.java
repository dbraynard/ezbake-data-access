...
/**
 * Wraps a real Driver with one that returns Connections decorated with
 * TokenPropertyConnectionDecorator.
 * <p />
 * Accepts any URL that a Driver is registered for, as long as the scheme is
 * prefixed with "ezbake". For example, jdbc:ezbake:postgresql://... wraps a
 * Driver that can accept jdbc:postgresql://.
 * <p />
 * The token passed to the database is configurable by the properties
 * <code>ezbake.token.provider</code> (the fully qualified name of a provider;
 * known options are:
 * <code>ezbake.data.postgres.SpringSecurityContextTokenProvider</code> -
 * default, <code>ezbake.data.postgres.ApplicationTokenProvider</code>, and
 * <code>ezbake.data.postgres.ExplicitTokenProvider</code>). If
 * <code>ExplicitTokenProvider</code> is used, then the property
 * <code>ezbake.token</code> must contain a base64 encoded EzSecurityToken.
 * <p />
 * The query parameters <code>ezbakeTokenProvider</code> and
 * <code>ezbakeToken</code> may also be passed to the connection URL. If
 * <code>ezbakeToken</code> is passed, it must be URL-escaped. Unfortunately,
 * different database implementations pass properties through the connection URL
 * differently (for example, Derby and H2 both pass pairs after a ';' instead of
 * '?'), so URL property passing is known to work only with PostgreSQL.
 * <p />
 * This driver manages EzBake security clients that can be used across different
 * connections. The properties passed to the client are taken from the
 * Properties passed to connect(). If not Properties are given, then they are
 * read using EzConfiguration.
 */
public class EzJdbcDriver implements Driver
{
    ...
    /**
     * Prefix for the scheme portion of the JDBC connection URL for this
     * EzBake-wrapped driver
     */
    public static final String JDBC_EZBAKE_URL_PREFIX = "jdbc:ezbake:";
    ...
    private static final Logger logger = LoggerFactory.getLogger(EzJdbcDriver.class);
    ...
    static
    {
        try
        {
            DriverManager.registerDriver(new EzJdbcDriver());
        }
        catch (SQLException e)
        {
            logger.error("Failed to register EzDriver with DriverManager", e);
        }
    }
    ...
    {
        if (!url.startsWith(JDBC_EZBAKE_URL_PREFIX))
        {
        ...
        Connection realConnection = GetConnectionFromRealDriver(unwrapURL(url), cloneProperties);

        return new ConnectionDecorator(realConnection, tokenProvider);
    }

    private Connection GetConnectionFromRealDriver(String url, Properties properties) throws SQLException
    {
       
        //eg. "jdbc:hive2://localhost:10000/default", "hadoop", "asdf");
        ...
        if (m.find())
        {
            databaseType = m.group();
        }
        else
        {
            throw new SQLException("EzDriver cannot determine database type in url connection string.");
        }
        
        if (databaseType.equalsIgnoreCase(HIVE))
        {
            return new org.apache.hive.jdbc.HiveDriver().connect(url, properties);
        }
        else if (databaseType.equalsIgnoreCase(POSTGRES))
        {
            return new org.postgresql.Driver().connect(url, properties);
        }
        else
        {
            throw new SQLException("EzDriver cannot find driver for database type found in url connection string");
        }    
    ...
    protected String unwrapURL(String url)
    {
        if (url.startsWith(JDBC_EZBAKE_URL_PREFIX))
        {
            return JDBC_URL_PREFIX + url.substring(JDBC_EZBAKE_URL_PREFIX.length());
        }
        else
        {
            return url;
    ...
    /**
     * Returns the requested token provider based on JDBC connection properties.
     * The properties are a combination of properties passed to the
     * DriverManager and are possibly overridden by query parameters in the JDBC
     * connection URL.
     *
     * @param properties JDBC connection properties
     * @return a token provider
     * @throws ClassNotFoundException if the token provider class is not found
     * @throws EzConfigurationLoaderException if the EzBake configuration cannot
     * be loaded
