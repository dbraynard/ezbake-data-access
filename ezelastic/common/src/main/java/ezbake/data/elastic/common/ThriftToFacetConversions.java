...
final class ThriftToFacetConversions {
    ...
    private static RangeFacetBuilder convertRangeFacet(
        ...
        for (final FacetRange range : facet.getRanges()) {
            if (range.getRangeType() == RangeType.INTEGER || range.getRangeType() == RangeType.DOUBLE) {
                if (range.isSetRangeFrom() && range.isSetRangeTo()) {
                    rf.addRange(Double.parseDouble(range.getRangeFrom()), Double.parseDouble(range.getRangeTo()));
                } else if (range.isSetRangeFrom()) {
                    rf.addUnboundedTo(Double.parseDouble(range.getRangeFrom()));
                } else if (range.isSetRangeTo()) {
                    rf.addUnboundedFrom(Double.parseDouble(range.getRangeTo()));
                }
            } else {
                if (range.isSetRangeFrom() && range.isSetRangeTo()) {
                    try {
                        rf.addRange(Long.valueOf(range.getRangeFrom()), Long.valueOf(range.getRangeTo()));
                    } catch (final NumberFormatException nfe) {
                        rf.addRange(range.getRangeFrom(), range.getRangeTo());
                    }
                } else if (range.isSetRangeFrom()) {
                    try {
                        rf.addUnboundedTo(Long.valueOf(range.getRangeFrom()));
                    } catch (final NumberFormatException nfe) {
                        rf.addUnboundedTo(range.getRangeFrom());
                    }
                } else if (range.isSetRangeTo()) {
                    try {
                        rf.addUnboundedFrom(Long.valueOf(range.getRangeTo()));
                    } catch (final NumberFormatException nfe) {
                        rf.addUnboundedFrom(range.getRangeTo());
                    }
                }
            }
        }
    ...
    private static RangeScriptFacetBuilder convertRangeScriptFacet(
        ...
        for (final FacetRange range : facet.getRanges()) {
            if (range.getRangeType() == RangeType.INTEGER || range.getRangeType() == RangeType.DOUBLE) {
                if (range.isSetRangeFrom() && range.isSetRangeTo()) {
                    rsf.addRange(Double.parseDouble(range.getRangeFrom()), Double.parseDouble(range.getRangeTo()));
                } else if (range.isSetRangeFrom()) {
                    rsf.addUnboundedTo(Double.parseDouble(range.getRangeFrom()));
                } else if (range.isSetRangeTo()) {
                    rsf.addUnboundedFrom(Double.parseDouble(range.getRangeTo()));
                }
            } else {
                if (range.isSetRangeFrom() && range.isSetRangeTo()) {
                    try {
                        rsf.addRange(Long.valueOf(range.getRangeFrom()), Long.valueOf(range.getRangeTo()));
                    } catch (final NumberFormatException nfe) {
                        rsf.addRange(Double.parseDouble(range.getRangeFrom()), Double.parseDouble(range.getRangeTo()));
                    }
                } else if (range.isSetRangeFrom()) {
                    try {
                        rsf.addUnboundedTo(Long.valueOf(range.getRangeFrom()));
                    } catch (final NumberFormatException nfe) {
                        rsf.addUnboundedTo(Double.parseDouble(range.getRangeFrom()));
                    }
                } else if (range.isSetRangeTo()) {
                    try {
                        rsf.addUnboundedFrom(Long.valueOf(range.getRangeTo()));
                    } catch (final NumberFormatException nfe) {
                        rsf.addUnboundedFrom(Double.parseDouble(range.getRangeTo()));
                    }
                }
