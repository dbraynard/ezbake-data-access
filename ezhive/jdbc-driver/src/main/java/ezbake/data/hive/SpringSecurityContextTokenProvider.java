...
/**
 * A token provider that provides proxied user tokens from a security client.
 * Calls
 * {@link ezbake.security.common.core.EzSecurityClient#fetchTokenForProxiedUser()}
 * (int, int)} to obtain the token from the current Spring Security context.
 */
class SpringSecurityContextTokenProvider implements Provider<EzSecurityToken>
{
    ...
    /**
     * Constructs a new provider that gets proxied user tokens from the security
     * client.
     *
